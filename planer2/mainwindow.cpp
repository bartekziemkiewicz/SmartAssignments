...
void MainWindow::on_pushButton_3_clicked()
{
    ...
    if(!db.isOpen())
    {
        ...
        //wczytaj_plan_zajec(1);
        //QAbstractButton * b = przyciski_przydzialow->button(1);
        //qDebug()<<b->palette().background().color();
...
void MainWindow::add_to_table(int row, int column)
{
    ...
    if(przyciski_przydzialow->checkedButton())
    {
        ...
        wczytaj_plan_zajec(1);
wczytaj_plan_zajec(2);
        //qDebug()<<"checked"<<button->palette().background().color();

    }

}

void MainWindow::wczytaj_plan_zajec(int grupa)
{

    QSqlQuery zapytanie;


    zapytanie.exec("select dzien, id_przydzialu, od_godzina, sala from plan_zajec where id_przydzialu IN (SELECT grupa FROM przydzial_zajec WHERE grupa = " + QString::number(grupa) + ")" );


    while(zapytanie.next())
    {
        int column=zapytanie.value(0).toInt();
        int id_przydzialu=zapytanie.value(1).toInt();
        int row=zapytanie.value(2).toInt();

        QTableWidgetItem *item= new QTableWidgetItem;
        tabelki[grupa-1]->setItem(row-godzina_poczatkowa, column, item);

        QAbstractButton * button =przyciski_przydzialow->button(id_przydzialu);
        //item->setBackgroundColor("red");
        qDebug()<<id_przydzialu;
        item->setBackgroundColor(przyciski_przydzialow->button(id_przydzialu)->palette().background().color());
       // QAbstractButton * b = przyciski_przydzialow->button(1);
      //  qDebug()<<b->palette().background().color();
        item->setText(przyciski_przydzialow->button(id_przydzialu)->text());
